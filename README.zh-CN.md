...
- 为本机 Codex 项目手动指定图标，或按主题启用的素材库进行稳定的加权随机；手动项目分配始终只保存在当前设备。
...
主题也可以通过自包含的 `.cdstheme` 文件导出和导入。导出时可以附带当前主题可分享的素材库设置和自定义素材库，但项目发现记录和手动分配绝不会进入分享包；素材库也可以独立导入导出为 `.cdsicons`。媒体限制、分享规则、运行机制、数据目录和故障排查详见[使用指南](docs/USER_GUIDE.md)。
