...
- 维护不可删除的系统素材库和多个自定义素材库，支持 PNG、JPEG、WebP、安全 SVG、GIF、随机开关、优先级及独立 `.cdsicons` 导入导出。
- 为本机 Codex 项目和会话手动指定标记图标，或按主题启用的素材库进行稳定的加权无放回随机；缓存项目之间的随机图标不重复，同一项目下的随机会话图标不重复且避开项目图标，并可通过默认开启的全局开关统一显示或隐藏。
- 将完整主题导出为可移植的 `.cdstheme` 文件，可选附带启用的自定义素材库，并在任一受支持平台导入。
...
主题也可以通过自包含的 `.cdstheme` 文件导出和导入。导出时可以附带当前主题可分享的素材库设置和自定义素材库，但项目与会话发现记录、标题和手动分配绝不会进入分享包；素材库也可以独立导入导出为 `.cdsicons`。媒体限制、分享规则、运行机制、数据目录和故障排查详见[使用指南](docs/USER_GUIDE.md)。
...
- 项目 ID 与名称只从经过验证的 Codex 页面读取并在本机缓存；`.cdstheme` 和 `.cdsicons` 都不会包含项目缓存或手动图标分配。
