...
export function VideoTranscodeDialog({
  ...
  return <div className="theme-dialog-backdrop video-transcode-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) close() }}>
    ...
    >
      <header><span><SlidersHorizontal size={16} /></span><h2 id="video-transcode-title">{title}</h2><button type="button" title="关闭" disabled={busy} onClick={close}><X size={16} /></button></header>
      <form noValidate onSubmit={(event) => { event.preventDefault(); submit() }}>
        <div className="video-source-summary">
          ...
          {inspection.portable ? inspection.highLoad ? <em className="is-warning">高负载</em> : <em>兼容</em> : <em className="is-warning">需转换</em>}
        </div>

        {allowOriginal
          ? <div className="segmented-control video-import-modes" aria-label="视频导入方式">
            <button type="button" className={mode === 'original' ? 'active' : ''} aria-pressed={mode === 'original'} onClick={() => setMode('original')}>原样使用</button>
            <button type="button" className={mode === 'transcode' ? 'active' : ''} aria-pressed={mode === 'transcode'} onClick={() => setMode('transcode')}>转换后使用</button>
          </div>
          : !inspection.portable && <p className="video-transcode-notice">源视频不兼容，必须转换后使用。</p>}
        ...
        <fieldset className="video-transcode-fields" disabled={busy || mode === 'original'}>
          <label>
            ...
            <select value={resolution} onChange={(event) => setResolution(event.currentTarget.value as VideoResolutionPresetId)}>
              {VIDEO_RESOLUTION_PRESETS.map((preset) => {
                ...
                return <option key={preset.id} value={preset.id} disabled={disabled}>{preset.label}</option>
              })}
              <option value="custom">自定义</option>
            </select>
          </label>
          {resolution === 'custom' && <div className="video-custom-resolution" aria-label="自定义分辨率">
            <label><span>宽度</span><input type="number" min={2} max={inspection.width} step="any" value={customWidth} onInput={(event) => updateCustomWidth(event.currentTarget.value)} onChange={(event) => updateCustomWidth(event.currentTarget.value)} /></label>
            <span aria-hidden="true">×</span>
            <label><span>高度</span><input type="number" min={2} max={inspection.height} step="any" value={customHeight} onInput={(event) => updateCustomHeight(event.currentTarget.value)} onChange={(event) => updateCustomHeight(event.currentTarget.value)} /></label>
          </div>}

          <label>
            <span>帧率</span>
            <select value={frameRateChoice} onChange={(event) => setFrameRateChoice(event.currentTarget.value as FrameRateChoice)}>
              <option value="source">原始（{formatNumber(inspection.frameRate)} FPS）</option>
          ...
          {frameRateChoice === 'custom' && <label className="video-custom-value"><span>自定义帧率</span><div><input type="number" min={1} max={240} step="any" value={customFrameRate} onInput={(event) => setCustomFrameRate(event.currentTarget.value)} onChange={(event) => setCustomFrameRate(event.currentTarget.value)} /><span>FPS</span></div></label>}

          <label>
            <span>视频码率</span>
            <select value={bitRateChoice} onChange={(event) => setBitRateChoice(event.currentTarget.value as BitRateChoice)}>
              <option value="auto">自动（CRF 23）</option>
              {VIDEO_BIT_RATE_PRESETS.map((value) => <option key={value} value={String(value)}>{value} Mbps</option>)}
              <option value="custom">自定义</option>
            </select>
          </label>
          {bitRateChoice === 'custom' && <label className="video-custom-value"><span>自定义码率</span><div><input type="number" min={MIN_VIDEO_BIT_RATE / 1_000_000} max={MAX_VIDEO_BIT_RATE / 1_000_000} step="any" value={customBitRate} onInput={(event) => setCustomBitRate(event.currentTarget.value)} onChange={(event) => setCustomBitRate(event.currentTarget.value)} /><span>Mbps</span></div></label>}
        </fieldset>

        {mode === 'transcode' && <div className="video-output-summary" aria-live="polite">
          <span>输出</span>
          <strong>{output ? `${output.width}×${output.height} · ${formatNumber(frameRate)} FPS · ${videoBitRate === null ? '自动码率' : `${formatNumber(videoBitRate / 1_000_000)} Mbps`}` : '参数无效'}</strong>
        </div>}
        {mode === 'transcode' && settingsError && <p className="theme-dialog-error" role="alert">{settingsError}</p>}
        {mode === 'transcode' && bitRateWarning && !settingsError && <p className="video-transcode-notice">目标码率高于源视频，不会增加已有画质。</p>}
        {error && <p className="theme-dialog-error" role="alert">{error}</p>}

        <footer>
          <button className="secondary-command" type="button" disabled={busy} onClick={close}>取消</button>
          <button className="primary-button" type="submit" disabled={busy || (mode === 'transcode' && Boolean(settingsError))}>
            ...
            {busy ? '处理中' : mode === 'original' ? '原样导入' : purpose === 'import' ? '转换并导入' : '生成优化版'}
          </button>
        </footer>
      </form>
    </section>
...
function validateSettings(settings: VideoTranscodeSettings, inspection: VideoAssetInspection, purpose: 'import' | 'optimize'): string | null {
  if (!Number.isFinite(settings.maxWidth) || !Number.isFinite(settings.maxHeight) || settings.maxWidth < 2 || settings.maxHeight < 2 || settings.maxWidth > 4096 || settings.maxHeight > 4096) return '分辨率必须在 2–4096px 之间。'
  if (!Number.isFinite(settings.frameRate) || settings.frameRate < 1 || !isVideoFrameRateAllowed(settings.frameRate, inspection.frameRate)) return `帧率必须在 1–${formatNumber(inspection.frameRate)} FPS 之间。`
  if (settings.videoBitRate !== null && (!Number.isFinite(settings.videoBitRate) || settings.videoBitRate < MIN_VIDEO_BIT_RATE || settings.videoBitRate > MAX_VIDEO_BIT_RATE)) return '视频码率必须在 0.5–100 Mbps 之间。'
  if (!isVideoTranscodeWorkloadAllowed(inspection, settings)) return VIDEO_TRANSCODE_WORKLOAD_MESSAGE
  if (purpose === 'optimize' && !isMeaningfulVideoOptimization(inspection, settings)) return VIDEO_OPTIMIZATION_NO_CHANGE_MESSAGE
  return null
}
