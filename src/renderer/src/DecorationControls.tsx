...
export function ComposerMelodyControls({ profile, assets, onChange, onInteractionEnd, onImportFont, mediaBusy = false, onSelectMedia }: DecorationControlsProps): React.JSX.Element {
  ...
  return <div className="decoration-controls" data-decoration-controls="composer-melody">
    <label className="toggle-row"><span>{t('显示输入框装饰')}</span><input type="checkbox" checked={config.visible} onChange={(event) => { const visible = event.currentTarget.checked; onChange((next) => { next.decorations.composerMelody.visible = visible }) }} /></label>
    <div className="segmented-control composer-decoration-modes" aria-label={t('输入框装饰内容')}><button className={config.mode === 'text' ? 'active' : ''} type="button" onClick={() => onChange((next) => { next.decorations.composerMelody.mode = 'text' })}>{t('文字')}</button><button className={config.mode === 'image' ? 'active' : ''} type="button" disabled={mediaBusy} onClick={() => selectMedia('image')}>{t('图片')}</button><button className={config.mode === 'gif' ? 'active' : ''} type="button" disabled={mediaBusy} onClick={() => selectMedia('gif')}>GIF</button></div>
    {config.mode === 'text' ? <>
      <div className="segmented-control melody-presets" aria-label={t('文字预设')}>{MELODY_PRESETS.map((preset) => <button className={activePreset === preset.id ? 'active' : ''} type="button" key={preset.id} onClick={() => onChange((next) => { next.decorations.composerMelody.text = preset.text })}>{t(preset.label)}</button>)}</div>
      <label className="quick-copy-field">{t('装饰文字')}<textarea value={config.text} maxLength={64} rows={2} onInput={(event) => { const text = event.currentTarget.value; onChange((next) => { next.decorations.composerMelody.text = text }, 'composer-melody-text') }} onBlur={onInteractionEnd} /></label>
      <label className="quick-copy-field">{t('文字动效')}<select value={config.effect} onChange={(event) => { const effect = event.currentTarget.value as ComposerDecorationEffect; onChange((next) => { next.decorations.composerMelody.effect = effect }) }}>{COMPOSER_DECORATION_EFFECT_IDS.map((effect) => <option value={effect} key={effect}>{t(EFFECT_LABELS[effect])}</option>)}</select></label>
      ...
      {trackEffect && <div className="segmented-control composer-decoration-directions" aria-label={t('移动方向')}><button className={config.direction === 'left' ? 'active' : ''} type="button" onClick={() => onChange((next) => { next.decorations.composerMelody.direction = 'left' })}><ArrowLeft size={14} />{t('向左')}</button><button className={config.direction === 'right' ? 'active' : ''} type="button" onClick={() => onChange((next) => { next.decorations.composerMelody.direction = 'right' })}><ArrowRight size={14} />{t('向右')}</button></div>}
      <div className="token-control"><AppearanceColorControl token="wave" value={melodyColor} onChange={(value) => onChange((next) => { next.appearance.colors.wave = value }, 'color-wave')} onChangeEnd={onInteractionEnd} />{profile.appearance.colors.wave && <button className="reset-token" type="button" title={t('恢复主题默认值')} onClick={() => onChange((next) => { delete next.appearance.colors.wave })}><RotateCcw size={12} /></button>}</div>
      ...
      <button className="asset-picker composer-decoration-asset-picker" type="button" disabled={mediaBusy} onClick={() => onSelectMedia?.(activeMediaMode)}>{mediaUrl ? <img src={mediaUrl} alt={t('输入框{media}装饰', { media: t(mediaLabel) })} /> : <Image size={20} />}<span><Upload size={13} />{t(mediaUrl ? replaceMediaLabel : selectMediaLabel)}</span></button>
      ...
      {config.source && <button className="secondary-command composer-decoration-remove" type="button" onClick={() => onChange((next) => { next.decorations.composerMelody.source = null; next.decorations.composerMelody.mode = 'text' })}><Trash2 size={14} />{t(removeMediaLabel)}</button>}
    </>}
    ...
    <label className="toggle-row"><span>{t('输入内容时隐藏')}</span><input type="checkbox" checked={config.hideWhenTyping} onChange={(event) => { const hideWhenTyping = event.currentTarget.checked; onChange((next) => { next.decorations.composerMelody.hideWhenTyping = hideWhenTyping }) }} /></label>
  </div>
}
...
export function HomeHeadingDecorationControls({ profile, onChange, onInteractionEnd, onImportFont }: DecorationControlsProps): React.JSX.Element {
  ...
  return <div className="decoration-controls" data-decoration-controls="home-heading">
    <label className="toggle-row"><span>{t('显示首页标题装饰')}</span><input type="checkbox" checked={config.visible} onChange={(event) => { const visible = event.currentTarget.checked; onChange((next) => { next.decorations.homeHeading.visible = visible }) }} /></label>
    <label className="quick-copy-field">{t('装饰文字')}<textarea value={config.text} maxLength={64} rows={2} onInput={(event) => { const text = event.currentTarget.value; onChange((next) => { next.decorations.homeHeading.text = text }, 'home-heading-decoration-text') }} onBlur={onInteractionEnd} /></label>
    <div className="token-control"><AppearanceColorControl token="homeHeadingDecoration" value={decorationColor} onChange={(value) => onChange((next) => { next.appearance.colors.homeHeadingDecoration = value }, 'color-home-heading-decoration')} onChangeEnd={onInteractionEnd} />{profile.appearance.colors.homeHeadingDecoration && <button className="reset-token" type="button" title={t('恢复主题默认值')} onClick={() => onChange((next) => { delete next.appearance.colors.homeHeadingDecoration })}><RotateCcw size={12} /></button>}</div>
