...
export function ParticleEffectControls({ profile, assets, onChange, onInteractionEnd, onImportIcon }: ParticleEffectControlsProps): React.JSX.Element {
  ...
  return <div className="decoration-controls particle-effect-controls" data-decoration-controls="sparkles">
    <label className="toggle-row"><span>{t('显示背景粒子')}</span><input type="checkbox" checked={config.visible} onChange={(event) => { const visible = event.currentTarget.checked; onChange((next) => { next.decorations.sparkles.visible = visible }) }} /></label>
    <fieldset className="particle-effect-settings" disabled={!config.visible} aria-label={t('粒子动效设置')}>
      <div className="segmented-control particle-effect-modes" aria-label={t('粒子动效')}>{PARTICLE_EFFECT_IDS.map((effect) => <button type="button" className={config.effect === effect ? 'active' : ''} aria-pressed={config.effect === effect} key={effect} onClick={() => onChange((next) => { next.decorations.sparkles.effect = effect })}>{t(PARTICLE_EFFECTS[effect].label)}</button>)}</div>
      <div className="segmented-control particle-performance-modes" aria-label={t('粒子渲染性能')}>{PARTICLE_PERFORMANCE_MODES.map((mode) => <button type="button" className={config.performanceMode === mode ? 'active' : ''} aria-pressed={config.performanceMode === mode} key={mode} onClick={() => onChange((next) => { next.decorations.sparkles.performanceMode = mode })}>{t(performanceLabels[mode])}</button>)}</div>
      ...
      <div className="token-control"><AppearanceColorControl token="sparkle" value={mainColor} onChange={(value) => onChange((next) => { next.appearance.colors.sparkle = value }, 'color-sparkle')} onChangeEnd={onInteractionEnd} />{profile.appearance.colors.sparkle && <button className="reset-token" type="button" title={t('恢复主题默认值')} onClick={() => onChange((next) => { delete next.appearance.colors.sparkle })}><RotateCcw size={12} /></button>}</div>
      <div className="sparkle-extra-colors">
        {config.extraColors.map((color, index) => <div className="sparkle-extra-color" key={index}><SolidColorControl label={t('附加颜色 {number}', { number: index + 1 })} value={color} onChange={(value) => onChange((next) => { next.decorations.sparkles.extraColors[index] = value }, `sparkle-extra-color-${index}`)} onChangeEnd={onInteractionEnd} /><button className="mini-icon-button" type="button" title={t('删除附加颜色 {number}', { number: index + 1 })} onClick={() => onChange((next) => { next.decorations.sparkles.extraColors.splice(index, 1) })}><Trash2 size={13} /></button></div>)}
        <button className="add-stop-button" type="button" disabled={config.extraColors.length >= 3} onClick={() => onChange((next) => { const colors = next.decorations.sparkles.extraColors; colors.push(EXTRA_COLOR_DEFAULTS[colors.length] ?? '#20bcc3') })}><Plus size={13} />{t('添加颜色')}</button>
      </div>
