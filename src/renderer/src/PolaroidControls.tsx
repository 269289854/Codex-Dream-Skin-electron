...
export function PolaroidControls({ profile, polaroidUrl, mediaBusy = false, showAdvanced = false, onChange, onInteractionEnd, onSelectImage }: PolaroidControlsProps): React.JSX.Element {
  ...
  return <div className="polaroid-controls">
    <label className="toggle-row"><span>{t('显示拍立得')}</span><input type="checkbox" checked={polaroid.visible} onChange={(event) => { const visible = event.currentTarget.checked; onChange((next) => { next.polaroid.visible = visible }) }} /></label>
    <button className="asset-picker polaroid-asset-picker" type="button" disabled={mediaBusy} onClick={onSelectImage}>{polaroidUrl ? (polaroid.source?.kind === 'video' ? <VideoThumbnail src={polaroidUrl} style={{ transform: mediaFlipCssTransform(polaroid.mediaTransform) }} /> : <img src={polaroidUrl} alt={t('拍立得原图')} style={{ transform: mediaFlipCssTransform(polaroid.mediaTransform) }} />) : <Image size={20} />}<span><Upload size={13} />{t(polaroidUrl ? '更换拍立得媒体' : '选择拍立得媒体')}</span></button>
    {polaroid.source && polaroidUrl && <MediaFlipControls value={polaroid.mediaTransform} onChange={(field, value) => onChange((next) => { next.polaroid.mediaTransform[field] = value })} />}
    {polaroid.source?.kind === 'video' && <div className="media-playback-controls"><label className="toggle-row"><span>{t(polaroid.playback.autoplay ? '自动播放' : '点击播放')} </span><input type="checkbox" checked={polaroid.playback.autoplay} onChange={(event) => { const autoplay = event.currentTarget.checked; onChange((next) => { next.polaroid.playback.autoplay = autoplay }) }} /></label><label className="toggle-row"><span>{t('循环播放')}</span><input type="checkbox" checked={polaroid.playback.loop} onChange={(event) => { const loop = event.currentTarget.checked; onChange((next) => { next.polaroid.playback.loop = loop }) }} /></label><label className="toggle-row"><span>{polaroid.playback.sound ? <Volume2 size={14} /> : <VolumeX size={14} />}{t('声音')}</span><input type="checkbox" checked={polaroid.playback.sound} onChange={(event) => { const sound = event.currentTarget.checked; onChange((next) => { next.polaroid.playback.sound = sound; if (sound) next.hero.playback.sound = false }) }} /></label><Range label="音量" min={0} max={1} step={.01} value={polaroid.playback.volume} disabled={!polaroid.playback.sound} onChange={(value) => onChange((next) => { next.polaroid.playback.volume = value }, 'polaroid-volume')} onChangeEnd={onInteractionEnd} /></div>}
    <div className="segmented-control polaroid-mode-tabs" aria-label={t('拍立得显示模式')}>
      <button type="button" className={polaroid.mode === 'full' ? 'active' : ''} onClick={() => onChange((next) => { next.polaroid.mode = 'full' })}>{t('整图')}</button>
      <button type="button" className={polaroid.mode === 'fence' ? 'active' : ''} onClick={() => onChange((next) => { next.polaroid.mode = 'fence' })}>{t('四点围栏')}</button>
    </div>
    ...
    <div className={shadow.visible ? 'polaroid-shadow-controls' : 'polaroid-shadow-controls is-disabled'}>
      <label className="toggle-row"><span>{t('显示阴影')}</span><input type="checkbox" checked={shadow.visible} onChange={(event) => { const visible = event.currentTarget.checked; onChange((next) => { next.polaroid.style.shadow.visible = visible }) }} /></label>
