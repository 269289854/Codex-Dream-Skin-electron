...
export function PreviewQuickEditor({ target, profile, assets, heroUrl, polaroidUrl, position, popoverRef, onChange, onInteractionEnd, onSelectImage, onImportIcon, onImportFont, onStateChange, onMore, onClose }: PreviewQuickEditorProps): React.JSX.Element {
  ...
  return <section ref={popoverRef} className={position ? 'preview-edit-popover' : 'preview-edit-popover measuring'} data-placement={position?.placement ?? 'right'} role="dialog" aria-label={`${target.label}快捷配置`} style={{ left: position?.left ?? 0, top: position?.top ?? 0 }}>
    ...
    <div className="preview-edit-popover-body">
      ...
      {editor.kind === 'polaroid' && <><label className="toggle-row"><span>显示拍立得</span><input type="checkbox" checked={profile.polaroid.visible} onChange={(event) => { const visible = event.currentTarget.checked; onChange((next) => { next.polaroid.visible = visible }) }} /></label><button className="secondary-command" type="button" onClick={() => onSelectImage('polaroid')}><Image size={15} />{polaroidUrl ? '更换拍立得原图' : '选择拍立得原图'}</button><div className="segmented-control polaroid-mode-tabs" aria-label="拍立得显示模式"><button type="button" className={profile.polaroid.mode === 'full' ? 'active' : ''} onClick={() => onChange((next) => { next.polaroid.mode = 'full' })}>整图</button><button type="button" className={profile.polaroid.mode === 'fence' ? 'active' : ''} onClick={() => onChange((next) => { next.polaroid.mode = 'fence' })}>四点围栏</button></div><Range label="宽度" min={.08} max={.6} step={.01} value={profile.polaroid.placement.width} onChange={(value) => onChange((next) => { next.polaroid.placement.width = value }, 'polaroid-width')} onChangeEnd={onInteractionEnd} /><Range label="旋转" min={-45} max={45} step={1} value={profile.polaroid.placement.rotation} onChange={(value) => onChange((next) => { next.polaroid.placement.rotation = value }, 'polaroid-rotation')} onChangeEnd={onInteractionEnd} suffix="°" /></>}
