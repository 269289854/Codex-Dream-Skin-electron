...
export function AccountMenuBackgroundControls({ profile, backgroundUrl, mediaBusy = false, onChange, onInteractionEnd, onSelectMedia }: AccountMenuBackgroundControlsProps): React.JSX.Element {
  ...
  return <div className="account-menu-background-controls">
    <div className="segmented-control account-menu-background-modes" aria-label="账号菜单背景类型">
    ...
    {mediaMode && <>
      <button className="asset-picker account-menu-background-asset-picker" type="button" disabled={mediaBusy} onClick={() => onSelectMedia(background.mode === 'image' ? 'image' : 'gif')}>
        {backgroundUrl ? <img src={backgroundUrl} alt="账号菜单背景预览" /> : <Image size={20} />}
        <span><Upload size={13} />{backgroundUrl ? '更换背景素材' : '选择背景素材'}</span>
      </button>
      {background.source && <button className="secondary-command account-menu-background-remove" type="button" onClick={() => onChange((next) => { next.accountMenuBackground.mode = 'color'; next.accountMenuBackground.source = null })}><Trash2 size={14} />移除背景素材</button>}
