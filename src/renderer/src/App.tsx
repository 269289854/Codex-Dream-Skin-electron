...
function ConversationPreview({ profile, assets }: { profile: ThemeProfile; assets: Record<string, string> }): React.JSX.Element {
  ...
  return <div className="preview-conversation"><header className="preview-thread-header"><div><strong>完善主题编辑器</strong><span>{PREVIEW_PROJECT_NAME} · Miku</span></div></header><div className="preview-conversation-surface" data-preview-target="conversation-background" tabIndex={0} role="button" aria-label="编辑对话区域背景">
    ...
    <div className="preview-message-list"><article className={`preview-message user${profile.conversationBubbles.visible ? ' bubble' : ''}`} data-preview-target="conversation-user-message" tabIndex={0}><strong>你</strong><p>让预览里的每个元素都可以直接点击配置。</p></article><section className="preview-assistant-response"><article className={`preview-message assistant${profile.conversationBubbles.visible ? ' bubble' : ''}`} data-preview-target="conversation-codex-message" tabIndex={0}><strong>Codex</strong><p>已建立全界面外观令牌，并同步到 <a href="#preview-runtime">运行时主题</a>。颜色、渐变和字体会实时更新。</p></article><button className="preview-primary-command" data-preview-target="primary-button" type="button">查看改动</button></section></div><div className="preview-conversation-composer"><PreviewComposer profile={profile} assets={assets} /></div>
  </div></div>
}
