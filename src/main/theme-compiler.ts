...
export async function compileTheme(
  ...
  const css = `:root { ${buildThemeVariableDeclarations(profile)} }\n` +
    ...
    `.dream-polaroid { position: fixed; right: auto; left: ${percent(profile.polaroid.placement.x)}; top: ${percent(profile.polaroid.placement.y)}; width: ${percent(profile.polaroid.placement.width)}; height: auto; transform: rotate(${profile.polaroid.placement.rotation}deg);${polaroid && polaroidLayout ? ` aspect-ratio: ${polaroidLayout.aspectRatio}; background-image: url("${escapeCssUrl(polaroid)}"); background-size: ${polaroidLayout.backgroundSize}; background-position: ${polaroidLayout.backgroundPosition}; clip-path: ${polaroidLayout.clipPath ?? 'none'};` : ''}${showPolaroid ? '' : ' display: none !important;'} }\n` +
    `@media (max-width: ${profile.polaroid.placement.hideBelowWidth}px) { .dream-polaroid { display: none !important; } }\n`

  return {
    css,
    rendererPayload: JSON.stringify({ version: 9, profile, home: { actions: HOME_ACTIONS }, assets }).replace(/</g, '\\u003c'),
    assets
  }
}
