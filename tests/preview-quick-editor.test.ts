...
describe('preview quick editor', () => {
  ...
  it('edits particle effects, independent materials, speed, and visibility from one target', () => {
    ...
    if (!icon || !speed || !count || !addColor) throw new Error('Particle effect controls are missing.')
    expect([...container.querySelectorAll('button')].some((button) => button.textContent?.includes('重新排列'))).toBe(false)
    ...
    expect(profile.decorations.sparkles).toMatchObject({ effect: 'rain', speed: 1.5, count: 12, seed: 0, extraColors: ['#20bcc3'] })
  ...
  it('edits the home heading decoration text, font, visibility, color, and size', () => {
    const profile = createDefaultTheme('00000000-0000-4000-8000-000000000000')
    renderEditor(PREVIEW_TARGETS['home-heading-decoration'], profile)
    const text = container.querySelector<HTMLTextAreaElement>('[data-decoration-controls="home-heading"] textarea')
    const font = container.querySelector<HTMLSelectElement>('[data-font-slot="homeHeadingDecoration"] select')
    const size = [...container.querySelectorAll('.range-row')].find((row) => row.querySelector('span')?.textContent === '字号')?.querySelector<HTMLInputElement>('input')
    const visibility = container.querySelector<HTMLInputElement>('[data-decoration-controls="home-heading"] .toggle-row input')
    const color = container.querySelector<HTMLInputElement>('[data-color-token="homeHeadingDecoration"] .color-text-input')
    if (!text || !font || !size || !visibility || !color) throw new Error('Home heading decoration controls are missing.')
    act(() => {
      Object.getOwnPropertyDescriptor(browserWindow.HTMLTextAreaElement.prototype, 'value')?.set?.call(text, '自定义标题 ✦')
      text.dispatchEvent(new browserWindow.Event('input', { bubbles: true }) as unknown as Event)
      font.value = 'builtin:jetbrains-mono'
      font.dispatchEvent(new browserWindow.Event('change', { bubbles: true }) as unknown as Event)
      Object.getOwnPropertyDescriptor(browserWindow.HTMLInputElement.prototype, 'value')?.set?.call(size, '24')
      size.dispatchEvent(new browserWindow.Event('input', { bubbles: true }) as unknown as Event)
      Object.getOwnPropertyDescriptor(browserWindow.HTMLInputElement.prototype, 'value')?.set?.call(color, '#123456')
      color.dispatchEvent(new browserWindow.Event('input', { bubbles: true }) as unknown as Event)
      visibility.click()
    })
    expect(profile.decorations.homeHeading).toMatchObject({ text: '自定义标题 ✦', fontSize: 24, visible: false })
    expect(profile.typography.slots.homeHeadingDecoration).toEqual({ kind: 'builtin', id: 'jetbrains-mono' })
    expect(profile.appearance.colors.homeHeadingDecoration).toBe('#123456')
  })
})
