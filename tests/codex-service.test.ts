...
describe('CodexService operation queue', () => {
  ...
  it('keeps identical renderer payload versions stable and changes them with content', async () => {
    ...
    expect(first).toContain('"conversationBubbles":{"visible":false,"user":{"mode":"none","dataUrl":null,"slice":25,"frameWidth":24,"contentPadding":20}')
    expect(first).toContain('"codex":{"mode":"none","dataUrl":null,"slice":25,"frameWidth":24,"contentPadding":20}')
