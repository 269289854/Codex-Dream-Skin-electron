...
describe('Studio preview editing interaction', () => {
  ...
  it('edits, undoes, saves, and resets tool activity bubbles independently', async () => {
    ...
    if (!conversation) throw new Error('Conversation preview command is missing.')
    act(() => conversation.click())
    const toolActivity = container.querySelector<HTMLElement>('[data-preview-target="conversation-tool-activity"]')
    if (!toolActivity) throw new Error('Tool activity preview target is missing.')
    expect(toolActivity.classList.contains('bubble')).toBe(true)

    pointerDown(toolActivity)
    expect(container.querySelector('[role="dialog"]')?.getAttribute('aria-label')).toBe('工具活动快捷配置')
    const toggle = container.querySelector<HTMLInputElement>('[role="dialog"] .toggle-row input')
    if (!toggle) throw new Error('Tool activity visibility control is missing.')
    act(() => toggle.click())
    expect(container.querySelector('[data-preview-target="conversation-tool-activity"]')?.classList.contains('bubble')).toBe(false)
    expect(container.querySelector('[data-preview-target="conversation-user-message"]')?.classList.contains('bubble')).toBe(true)

    const undo = container.querySelector<HTMLButtonElement>('button[title="撤销"]')
    if (!undo) throw new Error('Undo command is missing.')
    act(() => undo.click())
    expect(container.querySelector('[data-preview-target="conversation-tool-activity"]')?.classList.contains('bubble')).toBe(true)

    const refreshed = container.querySelector<HTMLElement>('[data-preview-target="conversation-tool-activity"]')
    if (!refreshed) throw new Error('Tool activity preview target disappeared.')
    pointerDown(refreshed)
    const currentToggle = container.querySelector<HTMLInputElement>('[role="dialog"] .toggle-row input')
    const primary = container.querySelector<HTMLInputElement>('[role="dialog"] [data-color-token="conversationToolText"] .color-text-input')
    const muted = container.querySelector<HTMLInputElement>('[role="dialog"] [data-color-token="conversationToolMutedText"] .color-text-input')
    const normalPaint = container.querySelector<HTMLInputElement>('[role="dialog"] [data-paint-token="conversationToolBackground"] .gradient-stop .color-text-input')
    if (!currentToggle || !primary || !muted || !normalPaint) throw new Error('Tool activity appearance controls are missing.')
    act(() => {
      currentToggle.click()
      setInputValue(primary, '#123456')
      setInputValue(muted, '#607080')
      setInputValue(normalPaint, '#f5f7f9')
    })

    const hover = [...container.querySelectorAll<HTMLButtonElement>('[role="dialog"] .state-tabs button')].find((button) => button.textContent === '悬停')
    if (!hover) throw new Error('Tool activity hover state is missing.')
    act(() => hover.click())
    const hoverPaint = container.querySelector<HTMLInputElement>('[role="dialog"] [data-paint-token="conversationToolHoverBackground"] .gradient-stop .color-text-input')
    if (!hoverPaint) throw new Error('Tool activity hover paint is missing.')
    act(() => setInputValue(hoverPaint, '#e8edf2'))
  ...
  it('keeps particles and composer melody synchronized across home and conversation previews', async () => {
    ...
    const unrelatedIteration = new browserWindow.Event('animationiteration', { bubbles: true })
    Object.defineProperty(unrelatedIteration, 'animationName', { configurable: true, value: 'dream-particle-rain' })
    act(() => secondAnimated.dispatchEvent(unrelatedIteration as unknown as Event))
