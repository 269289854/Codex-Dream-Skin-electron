...
describe('runtime appearance compilation', () => {
  ...
  it('consumes every registered appearance variable in the runtime stylesheets', async () => {
    ...
    const framedPlanBubbleRule = css.match(/html\.codex-dream-skin\[data-dream-plan-bubble-frame="layered"\] \.dream-conversation-plan-bubble\s*\{([^}]*)\}/)?.[1]
    ...
    expect(framedPlanBubbleRule).toContain('overflow: visible !important')
