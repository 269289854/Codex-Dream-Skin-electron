...
describe('renderer injection template', () => {
  it('produces valid JavaScript with no unresolved markers', async () => {
    ...
    expect(template).toContain('[data-codex-composer-root]')
    expect(template).toContain('[data-composer-surface-variant]')
    expect(template).toContain('[data-composer-layout][data-composer-radius-variant]')
    ...
    expect(template).toContain('attributeFilter: ["data-state", "hidden", "aria-hidden", "lang", "data-response-annotation-conversation", "data-content-search-unit-key", "data-markdown-text-style"]')
    expect(template).toContain('[data-markdown-text-style="assistant-message"]')
