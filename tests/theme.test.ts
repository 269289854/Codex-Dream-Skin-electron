...
import { DEFAULT_BRAND_COPY, DEFAULT_BRAND_COPY_EN, DEFAULT_HOME_COPY, DEFAULT_HOME_COPY_EN, PROJECT_PLACEHOLDER, splitHeadingTemplate } from '../src/shared/home-layout'
import { ACCOUNT_MENU_ITEMS } from '../src/shared/account-menu'
import { BRAND_SIGNATURE_MODES, CONVERSATION_BUBBLE_PRESETS, createDefaultConversationBubbleCornerOffsets, createDefaultTheme, createThemeInputSchema, DEFAULT_THEME_COLORS, parseThemeProfile, THEME_COLOR_PRESETS, VIDEO_PAUSE_POLICIES } from '../src/shared/theme'
...
import { DEFAULT_SIDEBAR_COPY, DEFAULT_SIDEBAR_COPY_EN, DEFAULT_SIDEBAR_NAV_COPY, DEFAULT_SIDEBAR_NAV_COPY_EN, SIDEBAR_NAV_ITEMS } from '../src/shared/sidebar-layout'
...
describe('theme schema and compiler', () => {
  it('defaults and validates account menu color, image, and GIF backgrounds without raising the theme version', () => {
    ...
    expect(profile.version).toBe(32)
    expect(profile.icons.projectEnvironment).toEqual({ kind: 'builtin', name: 'laptop' })
    expect(profile.icons.projectBranch).toEqual({ kind: 'builtin', name: 'git-branch' })
    const versionThirtyOne = structuredClone(profile) as unknown as { version: number; icons: Record<string, unknown> }
    versionThirtyOne.version = 31
    delete versionThirtyOne.icons.projectEnvironment
    delete versionThirtyOne.icons.projectBranch
    expect(parseThemeProfile(versionThirtyOne).icons).toMatchObject({
  ...
  it('migrates version twenty-five search and version twenty-six composer tool icons', () => {
    const current = createDefaultTheme(id)
    const { conversationBubbles: _conversationBubbles, ...legacyWithoutBubbles } = current
    const legacyCurrent = { ...legacyWithoutBubbles, copy: current.copy['zh-CN'] }
    const { sidebarSearch: _sidebarSearch, ...legacyIcons } = current.icons
    const migrated = parseThemeProfile({ ...legacyCurrent, version: 25, icons: legacyIcons })

    expect(migrated.version).toBe(32)
    expect(migrated.icons.sidebarSearch).toEqual({ kind: 'builtin', name: 'search' })
    expect(migrated.icons.sidebarMode).toEqual(current.icons.sidebarMode)

    const { composerAdd: _composerAdd, composerMicrophone: _composerMicrophone, ...versionTwentySixIcons } = current.icons
    const migratedTwentySix = parseThemeProfile({ ...legacyCurrent, version: 26, icons: versionTwentySixIcons })
    expect(migratedTwentySix.icons.composerAdd).toEqual({ kind: 'builtin', name: 'plus' })
    expect(migratedTwentySix.icons.composerMicrophone).toEqual({ kind: 'builtin', name: 'mic' })

    const { composerAdd: _missing, ...missingComposerAdd } = current.icons
    expect(() => parseThemeProfile({ ...current, icons: missingComposerAdd })).toThrow()
    expect(() => parseThemeProfile({ ...current, icons: { ...current.icons, composerMicrophone: { kind: 'builtin' } } })).toThrow()
    expect(() => parseThemeProfile({ ...current, icons: { ...current.icons, sidebarSearch: { kind: 'builtin' } } })).toThrow()
    expect(() => parseThemeProfile({ ...current, icons: { ...current.icons, unknownComposerSlot: { kind: 'builtin', name: 'plus' } } })).toThrow()
  })

  it('migrates and strictly validates brand signature text, image, and GIF modes', async () => {
    const profile = createDefaultTheme(id)
    expect(profile.brandSignature).toEqual({ mode: 'text', source: null, mediaWidth: 96 })
    expect(BRAND_SIGNATURE_MODES).toEqual(['text', 'image', 'gif'])

    const legacy = structuredClone(profile) as unknown as Record<string, unknown>
    legacy.version = 24
    legacy.copy = structuredClone(profile.copy['zh-CN'])
    delete legacy.brandSignature
    delete legacy.conversationBubbles
    expect(parseThemeProfile(legacy)).toMatchObject({
      version: 32,
      copy: {
        'zh-CN': { brandSignature: profile.copy['zh-CN'].brandSignature },
        'en-US': { brandSignature: profile.copy['zh-CN'].brandSignature }
      },
      brandSignature: { mode: 'text', source: null, mediaWidth: 96 }
    })

    const imageSource = { asset: 'assets/brand-signature.png', kind: 'image' as const, mimeType: 'image/png' as const }
    const gifSource = { asset: 'assets/brand-signature.gif', kind: 'image' as const, mimeType: 'image/gif' as const }
    expect(parseThemeProfile({ ...profile, brandSignature: { mode: 'image', source: imageSource, mediaWidth: 32 } }).brandSignature.source).toEqual(imageSource)
    expect(parseThemeProfile({ ...profile, brandSignature: { mode: 'gif', source: gifSource, mediaWidth: 190 } }).brandSignature.source).toEqual(gifSource)
    expect(parseThemeProfile({ ...profile, brandSignature: { mode: 'text', source: gifSource, mediaWidth: 96 } }).brandSignature.source).toEqual(gifSource)
    expect(() => parseThemeProfile({ ...profile, brandSignature: { mode: 'image', source: null, mediaWidth: 96 } })).toThrow('图片签名')
    expect(() => parseThemeProfile({ ...profile, brandSignature: { mode: 'image', source: gifSource, mediaWidth: 96 } })).toThrow('图片签名')
    expect(() => parseThemeProfile({ ...profile, brandSignature: { mode: 'gif', source: imageSource, mediaWidth: 96 } })).toThrow('GIF 签名')
    expect(() => parseThemeProfile({ ...profile, brandSignature: { mode: 'text', source: { asset: 'assets/signature.mp4', kind: 'video', mimeType: 'video/mp4' }, mediaWidth: 96 } })).toThrow('图片或 GIF')
    expect(() => parseThemeProfile({ ...profile, brandSignature: { mode: 'text', source: null, mediaWidth: 31 } })).toThrow()
  ...
  it('migrates version twenty-one playback defaults and validates video variants', () => {
    const current = createDefaultTheme(id)
    const { videoPlayback: _videoPlayback, ...versionTwentyOne } = { ...current, copy: current.copy['zh-CN'] }
    const migrated = parseThemeProfile({ ...versionTwentyOne, version: 21 })
    expect(migrated).toMatchObject({ version: 32, videoPlayback: { pausePolicy: 'hidden' } })
    ...
    const reference = {
      ...
      videoVariants: {
        ...
        original: { asset: 'assets/hero.webm', mimeType: 'video/webm' as const, width: 3840, height: 2160, frameRate: 59.94 },
        optimized: { asset: 'assets/hero-optimized.mp4', mimeType: 'video/mp4' as const, width: 1920, height: 1080, frameRate: 30 }
      }
    }
    expect(parseThemeProfile({ ...current, hero: { ...current.hero, source: reference } }).hero.source).toEqual(reference)
    expect(() => parseThemeProfile({ ...current, hero: { ...current.hero, source: { ...reference, asset: 'assets/wrong.mp4' } } })).toThrow('Active video asset')
    expect(() => parseThemeProfile({ ...current, hero: { ...current.hero, source: { ...reference, mimeType: 'video/webm' } } })).toThrow('Active video MIME')
    expect(() => parseThemeProfile({ ...current, hero: { ...current.hero, source: { ...reference, videoVariants: { ...reference.videoVariants, optimized: { ...reference.videoVariants.optimized, frameRate: 0 } } } } })).toThrow()
  })

  it('defaults conversation and tool bubbles on and migrates legacy bubble settings', () => {
    const current = createDefaultTheme(id)
    const legacyCurrent = { ...current, copy: current.copy['zh-CN'] }
    const second = createDefaultTheme('22222222-2222-4222-8222-222222222222')
    expect(current.version).toBe(32)
    ...
    expect(current.conversationBubbles).toMatchObject({
      visible: true,
      user: { source: { kind: 'none' }, contentPadding: 20 },
      codex: { source: { kind: 'none' }, contentPadding: 20 },
      plan: { source: { kind: 'none' }, contentPadding: 20 }
    })
    expect(current.conversationBubbles.user).not.toBe(current.conversationBubbles.codex)
    expect(current.conversationBubbles.plan).not.toBe(current.conversationBubbles.codex)
    expect(current.conversationBubbles.user.cornerOffsets).toEqual(createDefaultConversationBubbleCornerOffsets())
    const versionThirty = structuredClone(current) as unknown as { version: number; conversationBubbles: { user: Record<string, unknown>; codex: Record<string, unknown>; plan: Record<string, unknown> } }
    versionThirty.version = 30
    delete versionThirty.conversationBubbles.user.cornerOffsets
    delete versionThirty.conversationBubbles.codex.cornerOffsets
    delete versionThirty.conversationBubbles.plan.cornerOffsets
    const migratedThirty = parseThemeProfile(versionThirty)
    expect(migratedThirty.version).toBe(32)
    expect(migratedThirty.conversationBubbles.user.cornerOffsets).toEqual(createDefaultConversationBubbleCornerOffsets())
    ...
    expect(parseThemeProfile({ ...current, conversationBubbles: { ...current.conversationBubbles, user: { ...current.conversationBubbles.user, cornerOffsets: boundaryOffsets } } }).conversationBubbles.user.cornerOffsets).toEqual(boundaryOffsets)
    expect(() => parseThemeProfile({ ...current, conversationBubbles: { ...current.conversationBubbles, user: { ...current.conversationBubbles.user, cornerOffsets: { ...boundaryOffsets, topLeft: { x: 33, y: 0 } } } } })).toThrow()
    expect(() => parseThemeProfile({ ...current, conversationBubbles: { ...current.conversationBubbles, user: { ...current.conversationBubbles.user, cornerOffsets: { topLeft: { x: 0, y: 0 } } } } })).toThrow()
    ...
    const { videoPlayback: _videoPlaybackEighteen, conversationBubbles: _conversationBubbles, ...versionEighteen } = legacyCurrent
    ...
    expect(migrated).toMatchObject({ version: 32, videoPlayback: { pausePolicy: 'hidden' }, conversationBubbles: { visible: true }, toolActivityBubbles: { visible: true } })
    ...
    const { videoPlayback: _videoPlaybackNineteen, toolActivityBubbles: _toolActivityBubbles, ...versionNineteen } = legacyCurrent
    ...
    const migratedTwentyThree = parseThemeProfile({ ...legacyCurrent, version: 23, conversationBubbles: { visible: false } })
    expect(migratedTwentyThree).toMatchObject({
      ...
      conversationBubbles: {
        ...
        plan: { source: { kind: 'none' } }
      }
    })
    const codexPaint = { kind: 'solid' as const, color: '#123456' }
    const codexHoverPaint = { kind: 'solid' as const, color: '#654321' }
    const legacyBubbleStyle = { source: { kind: 'none' as const }, fit: 'nineSlice' as const, slice: 25, frameWidth: 24, contentPadding: 20 }
    const versionTwentySevenBubbles = {
      visible: true,
      user: structuredClone(legacyBubbleStyle),
      codex: structuredClone(legacyBubbleStyle)
    }
    const migratedTwentySeven = parseThemeProfile({
      ...legacyCurrent,
      version: 27,
      appearance: {
        colors: { conversationText: '#102030', conversationLink: '#304050' },
        paints: { conversationMessage: codexPaint, conversationMessageHover: codexHoverPaint }
      },
      conversationBubbles: {
        ...versionTwentySevenBubbles,
        user: { ...versionTwentySevenBubbles.user, source: { kind: 'preset', presetId: 'moon-stars' } },
        codex: { ...versionTwentySevenBubbles.codex, source: { kind: 'preset', presetId: 'ocean-shell' } }
      }
    })
    expect(migratedTwentySeven).toMatchObject({
      version: 32,
      conversationBubbles: {
        user: { source: { kind: 'preset', presetId: 'moon-stars' } },
        codex: { source: { kind: 'preset', presetId: 'ocean-shell' } },
        plan: { source: { kind: 'none' } }
      },
      appearance: {
        colors: {
          conversationText: '#102030',
          conversationLink: '#304050',
          conversationPlanText: '#102030',
          conversationPlanLink: '#304050'
        },
        paints: {
          conversationMessage: codexPaint,
          conversationMessageHover: codexHoverPaint,
          conversationPlanMessage: codexPaint,
          conversationPlanMessageHover: codexHoverPaint
        }
      }
    })
    expect(migratedTwentySeven.conversationBubbles.plan).not.toBe(migratedTwentySeven.conversationBubbles.codex)
    expect(() => parseThemeProfile({ ...current, conversationBubbles: versionTwentySevenBubbles })).toThrow()
    expect(() => parseThemeProfile({ ...current, conversationBubbles: { ...current.conversationBubbles, plan: { ...current.conversationBubbles.plan, source: { kind: 'preset', presetId: 'unknown' } } } })).toThrow()
    const preset = CONVERSATION_BUBBLE_PRESETS[0]
    expect(parseThemeProfile({
      ...current,
      conversationBubbles: {
        ...current.conversationBubbles,
        user: { ...current.conversationBubbles.user, source: { kind: 'preset', presetId: preset.id }, contentPadding: 20 }
      }
    ...
    expect(() => parseThemeProfile({ ...current, conversationBubbles: { ...current.conversationBubbles, user: { ...current.conversationBubbles.user, fit: 'stretch' } } })).toThrow()
    ...
    const corners = Object.fromEntries(['topLeft', 'topRight', 'bottomRight', 'bottomLeft'].map((corner) => [corner, {
      reference: { asset: `assets/${corner}.png`, kind: 'image', mimeType: 'image/png' },
      width: 256,
      height: 256
    }]))
    const customSource = { kind: 'custom', corners, borderColor: '#123456', borderWidth: 2, borderRadius: 16, ornamentSize: 56, ornamentOutset: 4 }
    expect(parseThemeProfile({ ...current, conversationBubbles: { ...current.conversationBubbles, user: { ...current.conversationBubbles.user, source: customSource, contentPadding: 24 } } }).conversationBubbles.user.source).toMatchObject({ kind: 'custom', borderColor: '#123456' })
    expect(() => parseThemeProfile({ ...current, conversationBubbles: { ...current.conversationBubbles, user: { ...current.conversationBubbles.user, source: { ...customSource, borderWidth: 5 }, contentPadding: 24 } } })).toThrow()
    expect(() => parseThemeProfile({ ...current, conversationBubbles: { ...current.conversationBubbles, user: { ...current.conversationBubbles.user, source: { ...customSource, corners: { ...corners, topLeft: { ...corners.topLeft, reference: { asset: 'assets/bubble.gif', kind: 'image', mimeType: 'image/gif' } } } }, contentPadding: 24 } } })).toThrow('PNG 或 WebP')

    const migratedTwentyNine = parseThemeProfile({
      ...current,
      version: 29,
      conversationBubbles: {
        visible: true,
        user: { ...legacyBubbleStyle, source: { kind: 'preset', presetId: 'daisy-heart' } },
        codex: { ...legacyBubbleStyle, source: { kind: 'custom', reference: { asset: 'assets/legacy.gif', kind: 'image', mimeType: 'image/gif' } } },
        plan: { ...legacyBubbleStyle, source: { kind: 'none' } }
      }
    })
    expect(migratedTwentyNine).toMatchObject({
      version: 32,
      conversationBubbles: {
        user: { source: { kind: 'preset', presetId: 'daisy-heart' } },
        codex: { source: { kind: 'none' } },
        plan: { source: { kind: 'none' } }
      }
    })
    ...
    ;(versionTwenty as unknown as Record<string, unknown>).copy = structuredClone(current.copy['zh-CN'])
    delete (versionTwenty as unknown as Record<string, unknown>).videoPlayback
    delete versionTwenty.decorations.sparkles.performanceMode
    expect(parseThemeProfile(versionTwenty).decorations.sparkles.performanceMode).toBe('balanced')
  })

  it('migrates version fifteen section titles with theme-following defaults', () => {
    const current = createDefaultTheme(id)
    const { videoPlayback: _videoPlayback, ...versionFifteen } = { ...current, copy: current.copy['zh-CN'] }
    ...
    expect(() => parseThemeProfile({ ...current, copy: { ...current.copy, 'zh-CN': { ...current.copy['zh-CN'], sidebarProjectsTitle: ' ' } } })).toThrow()
    expect(() => parseThemeProfile({ ...current, copy: { ...current.copy, 'zh-CN': { ...current.copy['zh-CN'], sidebarTasksTitle: '字'.repeat(81) } } })).toThrow()
  ...
  it('repairs version sixteen generated title colors while preserving identifiable edits', () => {
    ...
    const { videoPlayback: _videoPlayback, ...versionSixteen } = { ...generated, copy: generated.copy['zh-CN'] }
  ...
  it('migrates version twelve sidebar defaults and validates independent navigation settings', () => {
    ...
    expect(migrated.version).toBe(32)
    expect(migrated.copy['zh-CN']).toMatchObject({ ...DEFAULT_SIDEBAR_COPY, ...DEFAULT_SIDEBAR_NAV_COPY })
    expect(migrated.copy['en-US']).toEqual(migrated.copy['zh-CN'])
    ...
    expect(migratedThirteen.version).toBe(32)
    ...
    expect(() => parseThemeProfile({ ...current, copy: { ...current.copy, 'zh-CN': { ...current.copy['zh-CN'], sidebarNavSites: ' ' } } })).toThrow()
    expect(() => parseThemeProfile({ ...current, copy: { ...current.copy, 'zh-CN': { ...current.copy['zh-CN'], sidebarNavSites: '字'.repeat(81) } } })).toThrow()
    ...
    const second = createDefaultTheme('22222222-2222-4222-8222-222222222222')
    current.copy['zh-CN'].sidebarNavSites = '自定义站点'
    ...
    expect(second.copy['zh-CN'].sidebarNavSites).toBe(DEFAULT_SIDEBAR_NAV_COPY.sidebarNavSites)
  ...
  it('validates current themes and migrates version zero through nine profiles', () => {
    ...
    const expectedEnglishCopy = { ...DEFAULT_HOME_COPY_EN, ...DEFAULT_BRAND_COPY_EN, ...DEFAULT_SIDEBAR_COPY_EN, ...DEFAULT_SIDEBAR_NAV_COPY_EN }
    expect(parseThemeProfile(current).version).toBe(32)
    ...
    const currentWithLegacyCopy = { ...current, copy: current.copy['zh-CN'] }
    const { resetColors: _resetColors, videoPlayback: _videoPlayback, ...legacyCurrent } = currentWithLegacyCopy
    ...
    const { videoPlayback: _videoPlaybackSeventeen, windowBackground: _windowBackground, ...versionSeventeen } = currentWithLegacyCopy
    ...
    const { videoPlayback: _videoPlaybackFourteen, ...versionFourteenCurrent } = currentWithLegacyCopy
    const migratedFourteen = parseThemeProfile({
      ...versionFourteenCurrent,
      version: 14,
    ...
    expect(migratedFourteen.version).toBe(32)
    ...
    expect(migratedEight.version).toBe(32)
    ...
    expect(migratedSeven.version).toBe(32)
    ...
    expect(migratedSix.version).toBe(32)
    ...
    expect(migratedFour.version).toBe(32)
    ...
    expect(migratedFive.version).toBe(32)
    ...
    expect(migratedThree.version).toBe(32)
    expect(migratedThree.copy).toEqual({ 'zh-CN': expectedCopy, 'en-US': expectedCopy })
    ...
    expect(migratedTwo.version).toBe(32)
    ...
    expect(migratedOne.version).toBe(32)
    expect(migratedOne.name).toBe('已有主题')
    expect(migratedOne.copy).toEqual({ 'zh-CN': expectedCopy, 'en-US': expectedCopy })
    ...
    expect(migratedZero.version).toBe(32)
    ...
    expect(migratedZero.copy).toEqual({ 'zh-CN': expectedCopy, 'en-US': expectedEnglishCopy })
  ...
  it('migrates v11 media profiles with neutral flip defaults', () => {
    ...
    const { resetColors: _resetColors, videoPlayback: _videoPlayback, ...legacyCurrent } = { ...current, copy: current.copy['zh-CN'] }
  ...
  it('requires exactly one project placeholder and permits an empty subtitle', () => {
    const profile = createDefaultTheme(id)
    profile.copy['zh-CN'].subtitle = ''
    expect(parseThemeProfile(profile).copy['zh-CN'].subtitle).toBe('')
    expect(splitHeadingTemplate(`在 ${PROJECT_PLACEHOLDER} 中继续`)).toEqual({ before: '在 ', after: ' 中继续' })

    expect(() => parseThemeProfile({ ...profile, copy: { ...profile.copy, 'zh-CN': { ...profile.copy['zh-CN'], headingTemplate: '今天构建什么？' } } })).toThrow()
    expect(() => parseThemeProfile({ ...profile, copy: { ...profile.copy, 'zh-CN': { ...profile.copy['zh-CN'], headingTemplate: `${PROJECT_PLACEHOLDER} ${PROJECT_PLACEHOLDER}` } } })).toThrow()
    expect(() => parseThemeProfile({ ...profile, copy: { ...profile.copy, 'zh-CN': { ...profile.copy['zh-CN'], headingTemplate: `${PROJECT_PLACEHOLDER}${'字'.repeat(121)}` } } })).toThrow()
  })

  it('validates brand copy limits while permitting optional empty values', () => {
    const profile = createDefaultTheme(id)
    profile.copy['zh-CN'].brandSubtitle = ''
    profile.copy['zh-CN'].brandSignature = ''
    expect(parseThemeProfile(profile).copy).toEqual(profile.copy)

    expect(() => parseThemeProfile({ ...profile, copy: { ...profile.copy, 'zh-CN': { ...profile.copy['zh-CN'], brandTitle: ' ' } } })).toThrow()
    expect(() => parseThemeProfile({ ...profile, copy: { ...profile.copy, 'zh-CN': { ...profile.copy['zh-CN'], brandTitle: '字'.repeat(81) } } })).toThrow()
    expect(() => parseThemeProfile({ ...profile, copy: { ...profile.copy, 'zh-CN': { ...profile.copy['zh-CN'], brandSubtitle: '字'.repeat(121) } } })).toThrow()
    expect(() => parseThemeProfile({ ...profile, copy: { ...profile.copy, 'zh-CN': { ...profile.copy['zh-CN'], brandSignature: '字'.repeat(33) } } })).toThrow()
  })
  ...
  it('migrates version nine polaroids without changing their mode and validates appearance ranges', () => {
    ...
    const { resetColors: _resetColors, videoPlayback: _videoPlayback, ...legacyProfile } = { ...profile, copy: profile.copy['zh-CN'] }
  ...
  it('compiles deterministic assets without duplicating them into serialized fields', async () => {
    ...
    profile.copy['zh-CN'].headingTemplate = '<b>{project}</b>'
  ...
  it('does not read video media into the base64 image payload', async () => {
    ...
    profile.hero.source = {
      ...
      videoVariants: {
        active: 'optimized',
        original: { asset: 'assets/hero-original.webm', mimeType: 'video/webm', width: 3840, height: 2160, frameRate: 60 },
