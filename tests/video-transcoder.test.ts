...
describe('video transcoder', () => {
  ...
  it('rejects optimized output that violates the promised media contract', () => {
    ...
    expect(() => assertOptimizedVideoInspection(source, inspection({ width: 1920, height: 1080, frameRate: 30, codec: 'HEVC', portable: false, hasAudio: true }))).toThrow('编码')
    expect(() => assertOptimizedVideoInspection(source, inspection({ width: 1920, height: 1080, frameRate: 30, codec: 'AVC', audioCodec: null, hasAudio: false }))).toThrow('音轨')
  })
