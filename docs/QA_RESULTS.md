...
- `npm test`：49 个测试文件、545 项测试全部通过。
- `npm run test:config`：严格 UTF-8 配置安装和作用域恢复测试通过。
- `npm run build`：Node 与 Web TypeScript 检查、主进程、preload 和 renderer 生产构建通过。

本轮自动化测试覆盖主题模型和迁移、双语本地化与 IPC 错误、素材校验和持久化、视频预检与转码、主题分享、Studio 媒体协议、项目/会话图标与素材库持久化、v1/v2 到 v3 私有配置迁移、10,000 条会话缓存上限、稳定加权无放回分配、项目随机全局去重、同项目会话随机去重、手动占用排除、素材用尽回落、增量发现保持旧分配、会话树搜索和全局开关、原生项目/会话槽位渲染与清理、顶层页面切换后的预览尺寸重算、`.cdsicons` 导入导出、包含素材库的 `.cdstheme` v3 分享及项目/会话隐私排除、Codex 生命周期、Windows/macOS 平台驱动、应用更新、Studio UI 及双平台发布契约。
...
- 本轮随机图标去重改动未重新操作用户正在运行的 Studio/Codex 窗口；隔离 Electron 进程被单实例锁拒绝后立即退出。因此项目全局去重、同项目会话去重、素材用尽和动态新增只由共享分配、主进程存储、Studio UI 与运行时 DOM 自动化测试覆盖，没有新增真实环境截图。
