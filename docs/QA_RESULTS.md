...
- 通过经过身份验证的本机 CDP 会话连接 1 个真实 Codex 页面目标，刷新发现 7 个项目、46 个当前可见或临时展开后可见的会话；Studio 展开项目后正确显示会话图标和独立图标选择器。
- 项目与会话刷新使用 DOM 变化和不受后台计时器节流影响的截止检查，并与后台主题校验串行执行。自动化测试覆盖页面计时器不执行回调、后台校验并发、完整会话采集和状态恢复。
- 刷新前后保持 6 个项目折叠、1 个项目展开，所有 7 个项目列表均恢复为非“显示全部”状态。Codex 可以延迟卸载或保留已折叠项目的列表 DOM；刷新只以项目真实折叠状态判断恢复结果，不再将内部节点保留误报为脚本失败。
- 会话图标复用了标题左侧原生空槽，只添加类名、数据属性和 CSS 变量；探针确认 React 管理的原生子节点仍在原父节点内，清理后 Studio 属性和样式均已移除。
- 项目下会话行的起始内边距从 `8px` 调整为 `24px` 后，图标和标题右移 `16px`；项目行与会话整行的起点、宽度、左侧点击命中和选中背景均未变化，悬停背景仍覆盖整行，标题右边界与右侧操作按钮位置保持不变。移除会话图标标记后标题位置不变，确认全局图标开关不影响层级缩进。
...
- 当前真实 Codex 页面没有渲染独立“最近”区域会话，因此未取得该区域的实机对照数据；项目列表限定和通用会话行不带缩进由运行时 CSS 契约测试覆盖。
