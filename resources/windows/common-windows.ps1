...
function Get-DreamSkinCodexAppUserModelId {
  ...
  if ($packageFamilyName -notmatch '^[A-Za-z0-9._-]{1,200}$') {
    throw 'The registered Codex package family name is missing or invalid.'
  }
  return "$packageFamilyName!App"
}

function Initialize-DreamSkinApplicationActivator {
  if ('DreamSkin.ApplicationActivator' -as [type]) { return }
...
function Start-DreamSkinCodexProcess {
  ...
  try {
    return [DreamSkin.ApplicationActivator]::Activate($appUserModelId, $argumentLine)
  } catch {
    throw "Windows could not activate the registered Codex app package: $($_.Exception.Message)"
  }
}
